import { BrewingRecipe } from "#/models/data/recipe/brewing.ts";
...
import { Builder$SuspiciousStewEffectsComponent } from "#/models/data-components/suspicious-stew-effects.ts";
...
const stewEffects = (effect: string) => {
  const builder = new Builder$SuspiciousStewEffectsComponent();
  builder.effects((effects) => effects.effect(effect));
